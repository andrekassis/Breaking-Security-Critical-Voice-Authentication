...
from utils.adv_attack.reduce import SpectralGateNonStationary
...
class AttackerWrapper:
    ...
    def generate(self, x, y, p):
        ...
        if self.attack_type == "FFT_Attack" or self.attack_type == "TIME_DOMAIN_ATTACK" or self.attack_type == "STFT_Attack":
            return x, self.attack.generate(x, y, p)
...
def generate(adv, y, TFD, FD, TD, conf, sg, evalu=None, verbose = False):
    sg = SpectralGateNonStationary(**r_c)
    sg.set_len(adv.shape[1])
    TFD.attack.length = adv.shape[1]
    
    TD = configure_attack(TD, start=0.005, m=1, power=1, delta=None, alpha_factor=10, max_iter=30)
    TFD = configure_attack(TFD, start=0.005, m=1, power=1, delta=None, alpha_factor=10, max_iter=30)
    
    #adv = TD.generate(adv, y, 1)[1]
    adv = TFD.generate(adv, y, 1)[1]

    if verbose:
        print([ evalu[j].result(adv, 1 - np.argmax(y, axis=1), eval=True)[0] for j in range(len(evalu))])

    for k in tqdm(range(2 * conf['itrs'])):    
        m = np.min([k+1, 7])
        p= conf['prop_decrease'] / (m - 1.4 + conf['k_div']) #if k > 0 else None #(k+conf['k_div']-0.4)

        FD = configure_attack(FD, start=conf['FD'], m=m, power=0.75, delta=0, alpha_factor=1, max_iter=1)
        TFD = configure_attack(TFD, start=0.0007, m=m, power=0.75, delta=0, alpha_factor=1, max_iter=1)
        TD = configure_attack(TD, start=0.001, m=m, power=0.75, delta=0, alpha_factor=1, max_iter=1)

        adv = TFD.generate(adv, y, p)[1]
        #adv = TD.generate(adv, y, p)[1]
        adv = FD.generate(adv, y, p)[1]
    ...
    if conf['opt']:
        TD = configure_attack(TD, start=0.002, m=1, power=1, delta=0, alpha_factor=10, max_iter=30)
        adv = TD.generate(adv, y,None)[1]
...
def init(config, device):
    ...
    reduce_conf = {'win': 'kaiser_window', 'n_fft': 400, 'hop_length': 160, 'win_length': 400, 'padding':0, 'device': device}
    ...
    #TFD = get_attacker(config, attack_type = "STFT_Attack", system = "ADVCM", device = device) 
