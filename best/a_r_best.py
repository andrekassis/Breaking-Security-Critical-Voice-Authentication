...
def generate(adv, y, TFD, FD, TD, conf, evalu=None, verbose = False):
    ...
    TD.attack.epsilon = 0.005  
    TD.attack.alpha = TD.attack.epsilon / 10
    TD.attack.delta = 0 #None
    TD.attack.max_iter = 30
    adv = TD.generate(adv, y)[1]

    for k in tqdm(range(conf['itrs'])):    
        FD.attack.epsilon = np.power(conf['FD'] / (k+1), 1.2)#0.75)
        FD.attack.delta=0
        FD.attack.alpha = FD.attack.epsilon #/ 10
        FD.attack.max_iter = 1#0
        p= conf['prop_decrease'] / (k+conf['k_div']-0.4)
        r = (1 / (1-p) -1) / 2.5 + 1 #conf['r_div'] + 1
     
        
        TFD.attack.epsilon = 0.007 / (k+1) #0.006 / (k+1) #np.power(0.006 / (k+1), 0.75) #0.006 / (k+1)
        TFD.attack.alpha = TFD.attack.epsilon #/10
        TFD.attack.delta = 0
        TFD.attack.max_iter = 1#0
        
        
        TD.attack.epsilon = np.power(0.005 / (k+1), 1.2) #0.005 / (k+1)
        TD.attack.alpha = TD.attack.epsilon #/10
        TD.attack.delta = 0
        TD.attack.max_iter = 1#0
        ...
        if k > 0:
            adv = nr.reduce_noise(y = adv[0], sr=conf['sr'], stationary=False, n_fft = 64, prop_decrease=p) 
            adv = np.reshape(adv, (1, adv.shape[-1])).clip(-1, 1)         
