...
def generate(adv, y, TFD, FD, TD, conf, sg, evalu=None, verbose = False):
    ...
    TFD = configure_attack(TFD, start=0.005, m=1, power=1, delta=0, alpha_factor=10, max_iter=30)
    ...
    for k in tqdm(range(conf['itrs']*2)):    
        m = np.min([k+1, 7])
        p= 1 / (k+1 - 1.4 + conf['k_div']-0.1)

        FD = configure_attack(FD, start=conf['FD'], m=m, power=0.9, delta=0, alpha_factor=1, max_iter=1)
        TFD = configure_attack(TFD, start=0.0007, m=m, power=1, delta=0, alpha_factor=1, max_iter=1)
        TD = configure_attack(TD, start=0.0002, m=m, power=0.9, delta=0, alpha_factor=1, max_iter=1)
...
def init(config, device):
    ...
    reduce_conf = {'stationary': True, 'win': 'hann_window', 'n_fft': 1024, 'hop_length': 256, 'win_length': 1024, 'padding':0, 'device': device}
