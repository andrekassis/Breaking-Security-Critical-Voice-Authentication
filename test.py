...
def asv_cal_accuracies(test_loader, net, transform, label_fn, final_layer, batch_sz, len_eval, device):
    with torch.no_grad():
        probs = torch.empty(0, 3).to(device)
        #probs = torch.empty(71237, 3).to(device)
        for b, test_batch in enumerate(tqdm(test_loader)):
            ...
            #probs[b * batch_sz: b * batch_sz + row.shape[0], :] = row
    return probs.to('cpu')
...
if __name__ == '__main__':
    ...
    _, _, logits, model_dict = load_cm_asvspoof(args.config, 'cuda:3')
    ...
    test_loader = DataLoader(test_set, batch_size=args.bs, shuffle=False, num_workers = args.bs * 2, pin_memory=True) #num_workers=4)


    probabilities = asv_cal_accuracies(test_loader, Net, transform, label_fn, final_layer, args.bs, len(test_set), test_device)
