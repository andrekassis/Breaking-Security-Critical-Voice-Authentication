...
class OptimizerWrapper():
    def __init__(self, optimizer, sched):
        ...
        self.lr_scheduler = [ getattr(schedulers, sched['type'])(opt, **sched['params']) for opt in self.optimizer ]
...
def train(model, loss, optimizer, train_loader, val_loader, num_epochs, lr, out_f, 
                          eval_mode_for_validation, no_best_epoch_num, val_metric, drop_path_prob, 
                          transform, label_fn, final_layer, train_args, device):
...
if __name__ == "__main__":
    ...
    with Path(args.config).open('rt') as handle:
        config = json.load(handle, object_hook=OrderedDict)
        
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    protocol_file_path = os.path.join(os.path.join(args.base, 'labels'), config['training_parameters']['labels'])
    out = '/'.join(config['path'].split('/')[:-1])
    Path(out).mkdir(parents = True, exist_ok = True)
    f = open(os.path.join(out, 'log.log'), 'w')
    f.close()
    data_path = os.path.join(args.base, 'wavs/')

    _, _, logits, model_dict = load_cm_asvspoof(args.config, 'cpu', load_checkpoint = False)
    ...
    if config['training_parameters']['sampler'] == 'block_shuffle_by_length':
        with open(config['training_parameters']['lens_file'], 'r') as f:
            lengths = [ int(line.strip().split(' ')[1]) for line in f ]
        data_params['sampler'] = SamplerBlockShuffleByLen(lengths, data_params['batch_size'])
    ...
    optimizer = model.optimizer(config['training_parameters']['optimizer']['type'], 
                                   **config['training_parameters']['optimizer']['params'])
    optimizer_wrapper = OptimizerWrapper(optimizer, config['training_parameters']['scheduler'])
    ...
    model = train(model, loss, optimizer_wrapper, train_loader, val_loader, num_epochs, lr, out, 
                                  eval_mode_for_validation, no_best_epoch_num, val_metric, drop_path_prob, 
                                  transform, label_fn, final_layer, train_args, device)
