...
def generate(adv, y, TFD, FD, TD, conf, evalu=None, verbose = False): 
    ...
    for k in tqdm(range(conf['itrs'] + 40)):
        FD.attack.epsilon = np.power(2 * conf['FD'] / (k+2), 0.5)
        #TD.attack.epsilon = np.power(conf['FD'] / (k+2), 0.5) / 500
        TFD.attack.epsilon = np.power(conf['FD'] / (k+2), 0.5) / 300
        #FD.attack.delta = 0
    ...
    if conf['opt']:
        TD.attack.epsilon = 0.003
        adv = TD.generate(adv, y)[1].clip(-1, 1)
