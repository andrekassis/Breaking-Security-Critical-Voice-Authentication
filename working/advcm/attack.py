...
def generate(adv, y, TFD, FD, TD, conf, evalu=None, verbose = False): 
    ...
    TFD.attack.max_iter = 30
    adv = TFD.generate(adv, y)[1]
    add = 30

    for k in tqdm(range(conf['itrs']+add)):
        #break
        m = np.min([k+1, add//2 +1])
        ...
        if k > 0:
            adv = nr.reduce_noise(y = adv[0], sr=conf['sr'], stationary=False, n_fft = 200, prop_decrease=p) 
...
def init(config, device):
    ...
    #TD = get_attacker(config, attack_type = "auto_pgd", system = "ADVSR", device = device)
    FD = get_attacker(config, attack_type = "FFT_Attack", system = "ADVCM", device = device) 
