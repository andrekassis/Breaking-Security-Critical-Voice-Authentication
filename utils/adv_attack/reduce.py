...
class SpectralGate(ABC):
    ...
    def _stft(self, x):
        return torch.stft(x, n_fft = self._n_fft, hop_length=self._hop_length, 
    ...
    def _istft(self, x):
        return torch.istft(x, n_fft = self._n_fft, hop_length=self._hop_length,
                            win_length=self._win_length, window=self.win, center=True,
    ...
    def _do_filter(self, x, p, n_std_thresh_stationary=1.5):
        ...
        sig_mask = self._calc_mask(abs_sig_stft, n_std_thresh_stationary=n_std_thresh_stationary)
    ...
    def __call__(self, x, p = 1.0, n_std_thresh_stationary=1.5):
        ...
        filtered_padded_chunk = self._do_filter(padded, p, n_std_thresh_stationary=n_std_thresh_stationary)
...
class SpectralGateNonStationary(SpectralGate):
    ...
    def _set_len(self, length):
        ...
        rows = torch.tensor( [ 1/np.power(1-b, k) if k == 0 else b/np.power(1-b, k) for k in range(dim)])
    ...
    def _calc_mask(self, x, n_std_thresh_stationary=None):
        ...
        return 1 / (1 + torch.exp(-(sig_mult_above_thresh - \
                        self._thresh_n_mult_nonstationary) * self._sigmoid_slope_nonstationary))
...
class SpectralGateStationary(SpectralGate):
    ...
    def _calc_mask(self, x, n_std_thresh_stationary=1.5):
        sig_stft_db = 10 * torch.log10(torch.maximum(torch.tensor(1e-40, device = x.device), torch.abs(x) ** 2))
        sig_stft_db = torch.maximum(sig_stft_db, sig_stft_db.max() - 80.0)
        return sig_stft_db > (torch.mean(sig_stft_db, -1) + torch.std(sig_stft_db, -1) * n_std_thresh_stationary).t()
        
    def _apply_mask(self, sig_mask, p):
        sig_mask = sig_mask * p + torch.ones(sig_mask.shape, device = self.device).double() * (1.0 - p)
