...
class CM_Attack:
    def __init__(self, est, first_layer, last_layer, mid_layers, itrs, alpha_factors, powers, r_c, stop_reduce=10, r_div=3, optim_sat=15, stationary=False, verbose = True):
        r_c['device'] = est.device
        self.first_layer = getattr(base_attacks, first_layer['type'])(est, **first_layer['args'], r_c = r_c)
        self.last_layer = getattr(base_attacks, last_layer['type'])(est, **last_layer['args'], r_c = r_c)
        self.mid_layers = { m['type']: getattr(base_attacks, m['type'])(est, **m['args'], r_c = r_c) for m in mid_layers }
    ...
    def _configure_attacks(self, k):
        ...
        for m in self.mid_layers_conf:
            self.mid_layers[m['type']].epsilon = np.power(m['args']['epsilon'] / self.m, self.powers[m['type']])
            self.mid_layers[m['type']].alpha = self.mid_layers[m['type']].epsilon / self.alpha_factors[m['type']]
            self.mid_layers[m['type']].delta = m['args']['delta']
            self.mid_layers[m['type']].max_iter = m['args']['max_iter']
        
    def _reduce_noise(self, adv, k):
        return np.clip(self.sg(adv, p = self.p) * self.r, -1, 1) if k < self.stop_reduce else adv
