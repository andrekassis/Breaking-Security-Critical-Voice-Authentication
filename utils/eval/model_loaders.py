...
def load_cm_asvspoof(config, device, load_checkpoint=True, loss=None):
    with Path(config).open("rt", encoding="utf8") as handle:
...
def load_asv_plda(config, device, loss=None):
    ...
    if config["asv_args"]["fgmmfile"] is not None:
        fgmm = FullGMM(
            os.path.join(config["asv_args"]["base"], config["asv_args"]["fgmmfile"]),
            device,
        )
        extractor = ivectorExtractor(
            os.path.join(
                config["asv_args"]["base"], config["asv_args"]["ivector_extractor"]
            ),
            device,
        )
    else:
        ...
        extractor = xvectorModel(
            ...
            os.path.join(config["asv_args"]["base"], config["asv_args"]["transform"]),
            device,
        )
    plda_mdl = PLDA(
        os.path.join(config["asv_args"]["base"], config["asv_args"]["plda_mdl"]), device
    )
    ...
    SV_system = SVsystem(
        ...
        extractor,
        plda_mdl,
        os.path.join(config["asv_args"]["base"], config["asv_args"]["ivector_mean"]),
        device,
    )
    extractor = getattr(feats, config["extractor"]["fn"])(
        device=device, **config["extractor"]["args"]
...
def load_joint(config, device, loss=None):
    _, cm_eer, cm_l, cm = getattr(model_loaders, config["cm"]["loader"])(
        config["cm"]["config"], device, loss=loss
    )
    _, asv_eer, asv_l, asv = getattr(model_loaders, config["asv"]["loader"])(
        config["asv"]["config"], device, loss=loss
    )
    return (
        config["system"],
        asv_eer + cm_eer,
        [asv_l, cm_l],
        {
            "asv_args": asv,
            "cm_args": cm,
            "lambda_asv": config["asv"]["lambda"],
            "lambda_cm": config["cm"]["lambda"],
        },
    )
