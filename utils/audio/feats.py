...
class LFCC(torch.nn.Module):
    def __init__(self, fl, fs, fn, sr, filter_num, num_ceps, skip = True,
                 with_energy=False, with_emphasis=False,
                 compress = True, device = "cuda:0"):
...
class LFCC_Pad(LFCC, torch.nn.Module):
    def __init__(self, fl, fs, fn, sr, filter_num, num_ceps,
                 with_energy=False, with_emphasis=False,
                 skip = True, compress=True, device = "cuda:0", max_len = 750):
        torch.nn.Module.__init__(self)
        LFCC.__init__(self, fl = fl, fs = fs, fn = fn, sr = sr, filter_num = filter_num, num_ceps = num_ceps, skip = skip,
                 with_energy=with_energy, with_emphasis=with_emphasis,
                 compress = compress, device = device)
...
class lps(torch.nn.Module):
    def __init__(self, n_fft=128, hop_size=64, window_length=128, ref = 1.0, amin = 1e-30, power = 2, device = "cuda:0"):
        ...
        self.win = torch.blackman_window(self.window_length, device = device, requires_grad=False)
    ...
    def _calc(self, x):
        spec = torch.stft(x, self.n_fft, hop_length=self.hop_size, win_length=self.window_length, window=self.win,
                                    center=True, pad_mode='reflect', normalized=False, onesided=None, return_complex=True)
        S = torch.abs(spec) ** self.power
        S = self._transform(S)

        ret = 10 * torch.log10(torch.maximum(self.amin, S))
        ret = ret - 10.0 * (torch.log10(torch.maximum(self.amin, self.ref_value))).double()
        return ret
...
class ASVLps(lps, torch.nn.Module):
    def __init__(self, n_fft=128, hop_size=64, window_length=128, ref = 1.0, amin = 1e-30, power = 2, device = "cuda:0"):
        torch.nn.Module.__init__(self)
        lps.__init__(self, n_fft = n_fft, hop_size = hop_size, window_length = window_length, ref = ref, amin = amin, power = power, device = device)
...
class CMLps(lps, torch.nn.Module):
    def __init__(self, n_fft=1724, hop_size=130, window_length=1724, ref = 1.0, amin = 1e-30, max_len = 600, power = 2, device = "cuda:0"):
        torch.nn.Module.__init__(self)
        lps.__init__(self, n_fft = n_fft, hop_size = hop_size, window_length = window_length, ref = ref, amin = amin, power = power, device = device)
...
class mfcc(lps, torch.nn.Module):
    def __init__(self, sampling_rate = 16000, n_fft = 400, hop_size = 160, window_length = 400, num_mels = 30, fmin = 20., 
                                            fmax = 7600, ref = 1.0, amin = 1e-30, top_db = 80., n_mfcc = 24, power = 2, device = "cuda:0"):
        torch.nn.Module.__init__(self)
        lps.__init__(self, n_fft = n_fft, hop_size = hop_size, window_length = window_length, ref = ref, amin = amin, power = power, device = device)
        self.top_db= top_db
        self.n_mfcc = n_mfcc
        self.mel_basis = torch.tensor(librosa.filters.mel(sampling_rate, self.n_fft, n_mels = num_mels,
                                                                         fmin = fmin, fmax = fmax).astype(np.float32), device = device) 
        
        self.win = torch.hamming_window(self.window_length, device=device, requires_grad=False)
    ...
    def forward(self, audio):
        win = torch.tensor(np.array([-0.97, 1], dtype=np.float64).reshape((1, 1, 1, 2)), device = audio.device).type(torch.float32)
        x = audio.type(torch.float32)
        x = torch.reshape(F.pad(x, [1,1],'constant'), (x.shape[0], 1, 1, int(x.shape[1]) + 2))
        x = F.conv2d(x, win).squeeze(1).squeeze(1)[:, :-1]#exit(0) #.squeeze()[:, :-1]
        return self._mfcc(x)

class CQT(torch.nn.Module):
    def __init__(self, sr=16000, hop_length=256, n_bins=432, bins_per_octave=48, window="hann", fmin=15, truncate_len = 400, ref = 1.0, amin = 1e-30, device = "cuda:0"):
    ...
    def forward(self, x):
        win =  torch.tensor(np.array([-0.97, 1], dtype=np.float64).reshape((1, 1, 1, 2)), device = x.device).type(torch.float32)
        x = torch.reshape(F.pad(x, [1,1],'constant'), (x.shape[0], 1, 1, int(x.shape[1]) + 2))
    ...
    def _cqt(self, x):
        y = cqt(x, sr=self.sr, hop_length=self.hop_length, n_bins=self.n_bins, bins_per_octave=self.bins_per_octave, window=self.window, fmin=self.fmin).permute(0,2,1)
        y = torch.abs(y) ** 2
