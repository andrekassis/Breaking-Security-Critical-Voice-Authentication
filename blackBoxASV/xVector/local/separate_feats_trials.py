...
parser.add_argument(
    "--voiced-feats", type=str, default="data/voxceleb1_test/voiced_feats.scp"
)
...
with open(voiced_feats, "r") as feats_file:
    ...
    num_feats = len(feats)
    num_partation = (
        int(num_feats / num_per_part)
        if num_feats % num_per_part == 0
        else int(num_feats / num_per_part) + 1
    )
