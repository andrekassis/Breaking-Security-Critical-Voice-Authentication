...
class ivectorExtractor(object):
    def __init__(self, mdlfile, random=False):
            ...
            while line != "":
                if "<w_vec>" in line:
                    ...
                    self.extractor_matrix = torch.tensor(
                        extractor_matrix, device=device
                    )  # C*F*D
                    ...
                    for i in range(self.num_gaussian):
                        ...
                        for j in range(self.dim):
                            ...
                            line = rdfile.readline()
                    self.sigma_inv = torch.tensor(self.sigma_inv, device=device)
        ...
        self.aux2 = torch.matmul(
            torch.matmul(self.extractor_matrix.permute((0, 2, 1)), self.sigma_inv),
            self.extractor_matrix,
        )

        print("done extractor init")
    ...
    def Extractivector(self, zeroth_stats, first_stats):
        ...
        L += (zeroth_stats.unsqueeze(-1).unsqueeze(-1) * self.aux2).sum(
            0
        )  # Andre try optimize
        linear = (
            (torch.matmul(self.aux, first_stats.unsqueeze(-1))).sum(0).squeeze()
        )  # andre try optimize

        # for i in range(self.num_gaussian):
        #    L += zeroth_stats[i]*torch.matmul(torch.matmul(
        #        self.extractor_matrix[i].t(), self.sigma_inv[i]), self.extractor_matrix[i])
        #    linear += torch.matmul(torch.matmul(
        #        self.extractor_matrix[i].t(), self.sigma_inv[i]), first_stats[i])
    ...
    def DRV_L_inv(self, Ni_drv, L_inv):
        ...
        const_left = -1 * torch.matmul(
            torch.matmul(L_inv, self.T.t()), self.big_sigma_inv
        )  # D*CF
        const_right = torch.matmul(self.T, L_inv)  # CF*D

        L_inv_drv = []
        for i in range(self.dim):
            diag_elements = torch.matmul(
                torch.diag(Ni_drv[i]),
                torch.ones(self.num_gaussian, self.dim, device=device),
            ).view(
                1, -1
            )  # 1*CF
    ...
    def L_inv(self, post_seq):
        ...
        Ni = torch.diag(
            torch.matmul(
                torch.diag(Ni_vec),
                torch.ones(self.num_gaussian, self.num_gaussian, device=device),
            ).view(1, -1)
        )
        rightside = torch.matmul(
            torch.matmul(torch.matmul(self.T.t(), self.big_sigma_inv), Ni), self.T
        )
