...
from preprocess.generic import (
    load_wav_snf,
    stft,
    save_wav,
    revert_power_db_to_wav,
    uttid2wav,
    load_wav,
    save_wav_snf,
    extract_adv_voiced_feats,
)
...
def main(args):
    ...
    # print(1+args.sigma)
    keys, mat_list = extract_adv_voiced_feats(
        args.grads, args.vad, args.ori_feats, args.sigma
    )
    adv_featsfile = (
        "ark:| copy-feats ark: ark,scp:data/voxceleb1_test/spoofed_voiced_feats_sigma"
        + str(args.sigma)
        + ".ark,data/voxceleb1_test/spoofed_voiced_feats_sigma"
        + str(args.sigma)
        + ".scp"
    )
    utts_done = 0
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Adversarial Attack on GMM ivector+PLDA SV systems."
    )
    parser.add_argument(
        "--grads", default="data/voxceleb1_test/gradientfile.scp", type=str
    )
    parser.add_argument(
        "--ori-feats", default="data/voxceleb1_test/feats.scp", type=str
    )
