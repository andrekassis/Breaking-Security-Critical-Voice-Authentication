...
class Res2Net(nn.Module):
    ...
    def optimizer(self, opt, **params):
        optimizer = getattr(torch.optim, opt)(filter(lambda p: p.requires_grad, self.parameters()), **params)
        return [optimizer]
...
def Detector(MODEL_SELECT, GATE_REDUCTION=4):
    if MODEL_SELECT == 25:
        model = se_gated_linearconcat_res2net50_v1b(num_classes=2, KaimingInit=True, gate_reduction=GATE_REDUCTION)
    elif MODEL_SELECT == 31:
        model = se_gated_nonlinearconcat_res2net50_v1b(num_classes=2, KaimingInit=True, gate_reduction=GATE_REDUCTION)
    elif MODEL_SELECT == 5:
