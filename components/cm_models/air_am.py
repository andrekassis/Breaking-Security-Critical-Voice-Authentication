...
class AMSoftmax(nn.Module):
    def __init__(self, num_classes, enc_dim):
...
class AIR_AM(nn.Module):
    def __init__(self, num_nodes=3, enc_dim=256, resnet_type="18", nclasses=2):
        super(AIR_AM, self).__init__()
        self.score_fn = AMSoftmax(nclasses, enc_dim)
        self.model = ResNet(
    ...
    def load_state_dict(self, state_dict):
        fin = {"centers": state_dict["final.centers"]}
        self.score_fn.load_state_dict(fin)
        del state_dict["final.centers"]
    ...
    def optimizer(self, opt, **params):
        ...
        l_optimizer = getattr(torch.optim, opt[1])(self.score_fn.parameters(), 0.01)
        return [optimizer, l_optimizer]

    def save_state(self, path):
        state_dict = self.model.state_dict()
        state_dict["final.centers"] = self.score_fn.state_dict()["centers"]
