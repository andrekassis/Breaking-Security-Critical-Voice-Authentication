...
class ADVSR(BaseModel):
    ...
    def _extract_ivector(self, feats):
        ...
        trans_ivector = self.model.TransformIvectors(ivector, 1, simple_length_norm=False, normalize_length=True)
    ...
    def attack_pipeline(self, x, y, aggregate = True):   
        ...
        factor[ y == 1 ] = -1
        ret = loss * factor
        #print(torch.max(ret).detach().cpu().numpy())
        if aggregate:
            ret = torch.mean(ret)
            #ret = torch.max(ret)
    ...
    def get_score(self, x, ret_logits = True, aggregate=True):
        score = self.attack_pipeline(x, torch.tensor([1], device = x.device).repeat(x.shape[0]), False)
