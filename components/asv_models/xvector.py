...
class xvectorModel(nn.Module):
    def __init__(self, resume, transform, device='cuda:0'):
        ...
        l[-1] = l[-1][:-2]
        self.matrix = torch.tensor(np.array([ eval('[ ' + ','.join(line.split(' ')) + ' ]') for line in l]), device = device).float()
    ...
    def SubtractGlobalMean(self, ivector, mean):
        ...
        vec_out = torch.matmul(self.matrix[:, :x.shape[-1]],x.unsqueeze(-1)).squeeze(-1)
        if self.matrix.shape[-1] == x.shape[-1] + 1:
            vec_out += self.matrix[:, -1]

        if self.matrix.shape[-1] != x.shape[-1] + 1 and self.matrix.shape[-1] != vec_in.shape[-1]:
