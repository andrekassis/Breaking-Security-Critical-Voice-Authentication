...
class FullGMM(object):
            ...
            while line != '':
                if '<GCONSTS>' in line:
                    ...
                    for i in range(self.num_gaussians):
                        ...
                        line = rdfile.readline()
                    self.dim = len(data)
                    self.means_invcovars = torch.tensor(
                        means_invcovars, device=device)
                elif '<INV_COVARS>' in line:
                    self.invcovars = np.zeros((self.num_gaussians, self.dim, self.dim), dtype = np.float32) #uncomment Andre
                    for i in range(self.num_gaussians):
                        line = rdfile.readline()
    ...
    def Means(self):
        ...
        self.means = torch.matmul(torch.inverse(
            self.invcovars), self.means_invcovars.unsqueeze(-1)).squeeze(-1)
    ...
    def ComponentLogLikelihood(self, data):
        ...
        loglike -= 0.5*torch.matmul(mul.permute((0,3,1,2)), data.unsqueeze(-1)).squeeze(-1)
