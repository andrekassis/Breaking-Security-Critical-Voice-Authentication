...
class PLDA(object):
    ...
    def TransformIvector(self, ivector, num_examples, simple_length_norm, normalize_length):
        trans_ivector = torch.matmul(self.transform, (ivector-self.mean).unsqueeze(-1)).squeeze(-1)
        factor = 1.0
    ...
    def GetNormalizaionFactor(self, trans_ivector, num_examples):
        ...
        factor = torch.sqrt(self.dim / torch.matmul(inv_covar, trans_ivector_sq.unsqueeze(-1))).squeeze(-1)

        return factor

    def ComputeScores(self, trans_trainivector, num_examples, trans_testivector):
        mean = (num_examples*self.psi / (num_examples*self.psi+1.0)) * trans_trainivector
        ...
        loglike_given_class = -0.5*(logdet + torch.log(2*torch.tensor(
            3.1415926, device=self.device))*self.dim + torch.matmul(sqdiff, variance))
        ...
        loglike_without_class = -0.5*(logdet + torch.log(2*torch.tensor(
            3.1415926, device=self.device))*self.dim + torch.matmul(sqdiff, variance))
