...
class ivectorExtractor(object):
    ...
    def Extractivector(self, zeroth_stats, first_stats):
        L = (zeroth_stats.unsqueeze(-1).unsqueeze(-1) * self.aux2).sum(1) + torch.eye(
            self.ivector_dim, device=self.device
        )
        linear = (torch.matmul(self.aux, first_stats.unsqueeze(-1))).sum(1).squeeze(-1)
